            ...
            <div class="clearfix ">
                <div class="dis-center col-md-12 p-0 dis-center">
                    <ul class="nav nav-tabs">
                        ...
                        <li class="nav-item">
                            <a class="nav-link " data-toggle="tab" href="#schedule">Scheduled</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link " data-toggle="tab" href="#inprogress">InProgress</a>
                        </li>
                        <li class="nav-item">
        ...
        $('body').on('click', '.tripdetails', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('.service_category').text(item.service.servicesub_category ? item.service
                        .servicesub_category.service_subcategory_name : "NA");
                    $('.pricing').text(currency_symbol);
                    $('.payment_mode').text(item.payment ? item.payment.payment_mode : "NA");
                    if (item.before_image == null && item.after_image == null) {
                        $('.image_description').hide();
                    } else if (item.before_image == null) {
                        $('.before-img').attr('src',
                            "{{ asset('assets/layout/images/admin/no_image.png') }}");
                    ...
                    $('.pickup_date').text(new Date(item.schedule_at).toLocaleDateString("en-Us"));
                    $('.pickup_time').text(new Date(item.started_at).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit"
                    }));
            ...
            $.ajax({
                ...
                success: function(data) {
                   ...
                   `;
                    ...
                    $('.payment_mode').text(item.payment != null ? item.payment.payment_mode : 'NA');
                    $('.before-img').attr('src', item.before_image);
                    $('.after-img').attr('src', item.after_image);
                    $('.user-img').css('background-image', 'url(' + item.provider.picture + ')');
                    $('.provider_name').text(item.provider.last_name);
        ...
        trips_table = trips_table.DataTable({
            ...
            "columns": [{
                ...
                },*/
                {
                    "data": "provider_id",
                    render: function(data, type, row) {
                        return row.provider != null ? row.provider.first_name + ' ' + row.provider
                            .last_name : 'NA';
                    }
                },
                {
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        if (row.user_rated == 0 && row.status == 'COMPLETED') {
                            html += `<a href="{{ url('/user/service') }}/` + row.service_id +
                                `/service?id=` + row.id +
                                `"><span class="view-icon tripdetails" data-toggle="modal" data-target="#modal1" data-id = ` +
                                data +
                                ` data-toggle="tooltip" title="View"> <i class="fa fa-eye"></i></span></a>`;
                        } else {
                            html +=
                                `<span class="view-icon tripdetails" data-toggle="modal" data-target="#modal1" data-id = ` +
                                data +
                                ` data-toggle="tooltip" title="View"> <i class="fa fa-eye"></i></span>`;
                        }
                        html +=
                            ` <span class="view-icon dispute mt-1" data-toggle="modal" data-target="#disputeModal" data-id = ` +
                            data + ` data-user_id = ` + row.user_id + ` data-provider_id = ` + row
                            .provider_id +
                            ` data-toggle="tooltip" title="Create Dispute"><i class="fa fa-commenting-o" data-toggle="tooltip" title="Create Dispute"></i></span>`;
                        return html;
        ...
        upcoming_table = upcoming_table.DataTable({
            ...
            "columns": [{
                ...
                },
                {
                    "data": "schedule_at",
                    render: function(data, type, row) {
                        return new Date(data).toLocaleDateString("en-Us") + ' ' + new Date(data)
                            .toLocaleTimeString([], {
                                hour: "2-digit",
                                minute: "2-digit"
                            });
                ...
                },
                {
                    "data": "id",
                    render: function(data, type, row) {
                        return `<span class="view-icon upcomingdetails" data-toggle="modal" data-target="#modal2" data-id = ` +
                            data +
                            `> <i class="fa fa-eye"></i></span><span class="view-icon dispute m-1" data-toggle="modal" data-target="#disputeModal" data-id = ` +
