                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body ">
                            ...
                            <div class="col-md-6 float-right">
                                <div class="header-section">
                                    <div class="c-pointer dis-column" style="width:50%;">
                                        ...
                                        <div class="dis-center">
                                            <img class="beforeImage w-100 p-2" src="" class="w-100 p-2"
                                                alt="add_document">
                                        </div>
                                    </div>
                                    <div class="c-pointer dis-column" style="width:50%;">
                                        ...
                                        <div class="dis-center">
                                            <img class="afterImage w-100 p-2" src="" class="w-100 p-2"
                                                alt="add_document">
                                        </div>
                                    </div>
                                </div>
                                <div class="trip-user">
                                    <!-- For image listing -->
                                    <h4 class="modal-title m-0">User Details</h4>
                                    <hr />
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="from-to row m-0 getdisputedetails">
                            <div class="from">
                                <h5 class="text-left">Dispute Name : <span class ="dispute_name txt-yellow ml-2"></span>
                                </h5>
                        ...
                        <form class="validateForm getdispute" style= "color:red;">
                            ...
                            <div class="comments-section field-box mt-3 col-12" id ="dispute_comments">
                                ...
                                <textarea class="form-control" rows="4" cols="50" id="comments" name="comments"
                                    placeholder="Dispute Comments..."></textarea>
                            </div>
                            <!-- Dispute footer -->
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-primary btn-block">Submit <i class="fa fa-check"
...
@section('scripts')
    ...
    <script>
        ...
        $('#SERVICE').change(function() {
            if ($(this).is(":checked")) {
                window.location.replace('/provider/trips/{{ $type }}/service');
            }
        });
        // Header-Section
        function openNav() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data.responseData, function(key, item) {
                    $("#dispute_name").append('<option value="' + item.dispute_name + '">' + item
                        .dispute_name + '</option>');
                });
        ...
        $('.validateForm').validate({
            ...
            messages: {
                dispute_name: {
                    required: "Dispute Name is required."
                },
        ...
        trips_table = trips_table.DataTable({
            ...
            "lengthChange": false,
            "ajax": {
                "url": getBaseUrl() + "/provider/history/{{ $type }}/service",
                "type": "GET",
            ...
            "columns": [{
                ...
                },
                {
                    "data": "provider_id",
                    render: function(data, type, row) {
                        return row.payment != null ? row.user.currency_symbol + ' ' + row.payment.total :
                            'NA';
                    }
                },
                {
                ...
                },
                {
                    "data": "id",
                    render: function(data, type, row) {
                        return `<span class="view-icon providertripdetails" data-toggle="modal" data-target="#service_modal" data-id = ` +
                            data +
                            `> <i class="fa fa-eye"></i></span><span class="view-icon dispute mt-1" data-toggle="modal" data-target="#disputeModal" data-id = ` +
                            data + ` data-user_id = ` + row.user_id + ` data-provider_id = ` + row
                            .provider_id + `><i class="fa fa-commenting-o"></i></span>`;
                    }
                }
            ],
        ...
        $('body').on('click', '.providertripdetails', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('.map_key_img').attr('src',
                        "https://maps.googleapis.com/maps/api/staticmap?autoscale=1&size=420x380&maptype=terrian&format=png&visual_refresh=true&markers=icon:" +
                        window.url + "/assets/layout/images/common/marker-start.png%7C" + result
                        .s_latitude + "," + result.s_longitude + "&markers=icon:" + window.url +
                        "/assets/layout/images/common/marker-end.png%7C" + result.d_latitude + "," +
                        result.d_longitude + "&path=color:0x191919|weight:3|enc:" + result
                        .route_key + "&key=" + "{{ Helper::getSettings()->site->browser_key }}");
                    ...
                    $('.pickup_time').text(new Date(result.started_time).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit"
                    }));
                    $('.drop_date').text(new Date(result.finished_time).toLocaleDateString("en-Us"));
                    $('.drop_time').text(new Date(result.finished_time).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit"
                    }));
