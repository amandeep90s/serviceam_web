                ...
                <div class="card card-small">
                    <div class="card-header border-bottom">
                        ...
                        <a href="javascript:;" class="btn btn-success pull-right add"><i class="fa fa-plus"></i>
                            {{ __('admin.dispute.add_service_dispute') }}</a>
                    ...
                    <table id="data-table" class="table table-hover table_width display">
                        <thead>
                            <tr>
                                ...
                                <th data-value="service_request_id">{{ __('admin.dispute.dispute_request_id') }}</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', '.edit', function(e) {
                e.preventDefault();
                $.get("{{ url('admin/service-dispute/') }}/" + $(this).data('id') + "/edit", function(
                    data) {
                    $('.crud-modal .modal-container').html("");
                    $('.crud-modal .modal-container').html(data);
                });
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            return row.provider ? row.provider.first_name + " " + row.provider
                                .last_name : '';
                        }
                    },
                    {
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if (data) {
                                return data.replace(/^(.)|\s(.)/g, function(dispute_name) {
                                    return dispute_name.toUpperCase();
                                });
                            } else {
                                return data;
                            }
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if (data) {
                                return data.replace(/^(.)|\s(.)/g, function(comments) {
                                    return comments.toUpperCase();
                                });
                            } else {
                                return data;
                            }
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if (data == "user") {
                                return row.user ? row.user.currency_symbol + row.refund_amount :
                                    '$0';
                            }
                            return row.provider ? row.provider.currency_symbol + row.refund_amount :
                                '$0';
                        }
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if (row.status == "open" && "{{ Helper::getDemomode() }}" != 1)
                                return "<button data-id='" + data +
                                    "' class='btn btn-block btn-success edit'>Edit</button>";
                        }
                    }
