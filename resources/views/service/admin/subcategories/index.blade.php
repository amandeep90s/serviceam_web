...
@section('title') Service Sub categories @stop
                    ...
                    <div class="card-header border-bottom">
                        ...
                        @if (Helper::getDemomode() != 1)
                            <a href="javascript:;" class="btn btn-success pull-right add new_user"><i
                                    class="fa fa-plus"></i> {{ __('service.admin.subcategories.add') }}</a>
                        @endif
                    </div>
                    ...
                    <table id="data-table" class="table table-hover table_width display">
                        <thead>
                            <tr>
                                ...
                                <th data-value="service_subcategory_status">{{ __('service.admin.subcategories.status') }}
                                </th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', '.edit', function(e) {
                e.preventDefault();
                $.get("{{ url('admin/service-subcategories/') }}/" + $(this).data('id') + "/edit",
                    function(data) {
                        $('.crud-modal .modal-container').html("");
                        $('.crud-modal .modal-container').html(data);
                    });
            ...
            table = table.DataTable({
                ...
                "columns": [{
                    ...
                    {
                        "data": "service_subcategory_status",
                        render: function(data, type, row) {
                    ...
                    },
                    {
                        "data": function(data, type, row) {
                            ...
                            if ({{ Helper::getDemomode() }} != 1) {
                                button += '<li > <a class="dropdown-item status" data-id="' + data
                                    .id + '" data-value="' + data.service_subcategory_status +
                                    '"  href="javascript:;"><i class="fa fa-plus" aria-hidden="true" title="Status">&nbsp;' +
                                    status + '</i></a></li><li><a href="javascript:;" data-id="' +
                                    data.id +
                                    '" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li>';
                            }
                            button += '</ul> </li></div>';
                            return button;
