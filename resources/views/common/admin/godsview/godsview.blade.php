    ...
    <div class="container-fluid">
      <div class="box box-block bg-white">
              ...
              <ul class="nav nav-tabs " role="tablist" id="tab-button">
                <ul id="tab-button">
                  @foreach (Helper::getServiceList() as $service)
                    <li class="nav-item">
                      <a class="services" data-type="{{ strtolower($service) }}" data-toggle="tab"
                        href="#{{ strtolower($service) }}Tab" role="tab" aria-selected="true">{{ $service }}</a>
                    </li>
                  @endforeach
                </ul>
                <!-- <ul id="tab-button">
        ...
        <div class="tab-content">
          <div id="transportTab" class="tab-pane col-sm-12 col-md-12 col-lg-12 p-0" role="tabpanel">
            <div class="row p-2">
              <div class="col-md-12">
                <div id="floating-panel">
                  <button class="btn  godseye_menu" data-value="STARTED">Enroute to Pickup</button>
                  <button class="btn godseye_menu" data-value="ARRIVED">Reached Pickup</button>
                  <button class="btn godseye_menu" data-value="PICKEDUP">Journey Started</button>
                  <button class="btn godseye_menu" data-value="ACTIVE">Available</button>
                  <button class="btn godseye_menu btn-info" data-value="ALL">All</button>
                </div>
                ...
                <div class="row">
                  <div class="col-md-4">
                    <label class="provider_title ">All</label>
          ...
          <div id="serviceTab" class="tab-pane active col-sm-12 col-md-12 col-lg-12 p-0" role="tabpanel">
            <div class="row p-2">
              <div class="col-md-12">
                <div id="floating-panel">
                  <button class="btn  godseye_menu" data-value="ARRIVED">Reached Location</button>
                  <button class="btn godseye_menu" data-value="PICKEDUP">Started Service</button>
                  <button class="btn godseye_menu" data-value="DROPPED">End Service</button>
                  <button class="btn godseye_menu" data-value="ACTIVE">Available</button>
                  <button class="btn godseye_menu btn-info" data-value="ALL">All</button>
                </div>
                <br>
          ...
          <div id="orderTab" class="tab-pane col-sm-12 col-md-12 col-lg-12 p-0" role="tabpanel">
            <div class="row p-2">
              <div class="col-md-12">
                <div id="floating-panel">
                  <button class="btn  godseye_menu" data-value="STARTED">Enroute to Restaurant</button>
                  <button class="btn godseye_menu" data-value="REACHED">Reached Restaurant</button>
                  <button class="btn godseye_menu" data-value="PICKEDUP">Picked up Food</button>
                  <button class="btn godseye_menu" data-value="ARRIVED">Reached to Delivery Location</button>
                  <button class="btn godseye_menu" data-value="ACTIVE">Available</button>
                  <button class="btn godseye_menu btn-info" data-value="ALL">All</button>
                </div>
                <br>
...
@section('scripts')
  @parent
  <script
    src="https://maps.googleapis.com/maps/api/js?key={{ Helper::getSettings()->site->browser_key }}&libraries=visualization">
  </script>
  ...
  <script>
    ...
    function initialize(type, latitude = 0, longitude = 0) {
            ...
            if (providers.length > 0) {
              ...
              if (locations.length > 0) {
                for (i = 0; i < locations.length; i++) {
                  ...
                  var picture = (providers[i].picture == null || providers[i].picture == "") ?
                    "{{ asset('assets/layout/images/admin/avatar.jpg') }}" : providers[i].picture;
                  if (providers[i].is_assigned == '0') {
                    $.each(providers[i].providerservice, function(index, item) {
                      if (type == 'transport') {
                        mainservices += `<span class="font-weight-bold">` + item.maintransport.ride_name +
                          '></span><span>' + item.ride_vehicle.vehicle_name + `</span>,`;
                      }
                      if (type == 'service') {
                        var str = mainservices.indexOf(item.mainservice.service_category_name);
                        if (str == -1) {
                          mainservices += `<span class="font-weight-bold">` + item.mainservice
                            .service_category_name + `</span>,`;
                        }
                      }
                      if (type == 'order') {
                    ...
                    } else if (providers[i].request.status == 'STARTED' || providers[i].request.status ==
                      'ARRIVED' || providers[i].request.status == 'PICKEDUP' || providers[i].request.status ==
                      'REACHED' || providers[i].request.status == 'DROPPED') {

                      if (type == 'transport') {
                        mainservices += `<span class="font-weight-bold">` + providers[i].current_vehicle
                          .ride_vehicle.ride_type.ride_name + '></span><span>' + providers[i].current_vehicle
                          .ride_vehicle.vehicle_name + `</span>,`;
                      }
                      if (type == 'service') {
                        mainservices += `<span class="font-weight-bold">` + (providers[i].request.request.service
                          .service_category ? providers[i].request.request.service.service_category
                          .service_category_name : '') + `</span>,`;
                      }
                      if (type == 'order') {
                        mainservices += `<span class="font-weight-bold">` + providers[i].current_store_detail
                          .storetype.name + `</span>,`;
                      }
                  ...
                  if (status == 'ALL' || status == 'ACTIVE') {
                    if (providers[i].is_assigned == '0') {
                      var li = $(`<li style='margin-bottom: 19px;' id="` + providers[i].id + `">
                        ...
                        `">
                                  ...
                                  <div>
                                  <img class="avt img-circle" height="50px" height="50px" style="border-radius: 50%" src="` +
                        picture + `">
                                  </div>
                                  <p>` + providers[i].first_name + ` ` + providers[i].last_name + ` 
                                  </p><p>` + mainservices.replace(/,\s*$/, "") + `</p><p>` + protect_number(providers[
                          i].mobile) + `</p>
                    ...
                    var li = $(`<li style='margin-bottom: 19px;' id="` + providers[i].id + `">
                      ...
                      `">
                                ...
                                <div>
                                <img class="avt img-circle"  style="border-radius: 50%" height="50px" height="50px" src="` +
                      picture + `">
                                </div>
                                <p>` + providers[i].first_name + ` ` + providers[i].last_name + ` 
                                </p><p>` + mainservices.replace(/,\s*$/, "") + `</p><p>` + protect_number(providers[i]
                        .mobile) + `</p>
                              </li>`).on('click', onClick(marker));
      ...
      function showinfoWindow(marker) {
        ...
        if (status == 'ALL' || status == 'ACTIVE') {
          mainservices += `<a href="javascript:void(0)" data-sid="` + type + `"   data-id="` + provider.id +
            `" class=" vservice" ><i class="material-icons">remove_red_eye</i></a>`;


        } else if (provider.request.status == 'STARTED' || provider.request.status == 'ARRIVED' || provider.request
          .status == 'PICKEDUP' || provider.request.status == 'REACHED' || provider.request.status == 'DROPPED') {

          if (type == 'transport') {
            mainservices += `<span class="font-weight-bold">` + provider.current_vehicle.ride_vehicle.ride_type
              .ride_name + '></span><span>' + provider.current_vehicle.ride_vehicle.vehicle_name + `</span>,`;
          }
          if (type == 'service') {
            mainservices += `<span class="font-weight-bold">` + (provider.request.request.service.service_category ?
              provider.request.request.service.service_category.service_category_name : '') + `</span>,`;
          }
          if (type == 'order') {
        ...
        if (marker.provider.request != null) {
          ...
          var live_track = (Object.keys(marker.provider.request).length > 0) ? (marker.provider.request.status ==
              'PICKEDUP') ?
            `<tr><td></td><td><a href="{{ url('/track') }}/` + marker.provider.request.request_id +
            `" target="_blank"><b>Live tracking</b></a></td></tr>` : `` : ``;

          var picture = (marker.provider.picture == null || marker.provider.picture == "") ?
            "{{ asset('assets/layout/images/admin/avatar.jpg') }}" : marker.provider.picture;
          ...
          if (status == 'ALL' || status == 'ACTIVE') {
            ...
            html += `<tr><td>&nbsp;&nbsp;Services: </td><td>` + mainservices.replace(/,\s*$/, "") + `</td></tr>` +
              live_track +
              `</tbody>`;
          }
          html += `</table>`;
