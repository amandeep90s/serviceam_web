...
<div class="card-header border-bottom">
    ...
    <h6 class="m-0"style="margin:10!important;">Provider Approval</h6>
</div>
<div class="row p-3">
    ...
    <div class="col-md-8 box-card price_lists_sty priceBody">
        ...
        <div class="col-xs-12">
            <!-- <nav class="services">
                    <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                        <a class="nav-item nav-link active" id="nav-daily-tab" data-toggle="tab" href="#daily" role="tab" aria-controls="daily" aria-selected="true">Provider Services</a>
                    </div>
                </nav>
            ...
            <nav class="nav_document">
                <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                    <a class="nav-item nav-link active" id="nav-daily-tab" data-toggle="tab" href="#daily"
                        role="tab" aria-controls="daily" aria-selected="true">Provider Document</a>
                </div>
            </nav>
            ...
            <!--  <nav class="zone">
                    <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                        <a class="nav-item nav-link active" id="nav-daily-tab" data-toggle="tab" href="#daily" role="tab" aria-controls="daily" aria-selected="true">Zone</a>
                    </div>
                </nav> -->
            <!--  <div class="zone pricing-nav nav-wrapper" id="nav-tabContent">
                    ...
                    <div class="form_pad">
                        ...
                        <div class="form-group col-md-6 p-1">
                            ...
                            <button type="button" class="btn btn-accent update ">Update</button>
                            <button type="button" class="btn btn-danger cancel zcnl float-right">Cancel</button>
                        </div>
        ...
        <button type="button" class="btn btn-success  approve_all">Approve All</button>
        <button type="button" class="btn btn-danger cancel rld">Cancel</button>
...
<script>
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(data.responseData, function(key, item) {
                ...
                $("#zone_id").append('<option value="' + item.id + '" ' + selected + '>' + item
                    .name + '</option>');
            });
            ...
            success: function(datas) {
                ...
                if (providerdetails.providerservice.length > 0) {
                    ...
                    $.each(services, function(k, servicename) {
                        servicenames +=
                            `<a href="#" class="list-group-item cityActiveClass" onclick="getData('` +
                            servicename + `')" ><span>` + servicename.charAt(0).toUpperCase() +
                            servicename.slice(1)
                            .toLowerCase() + `</span></a>`;
                    });
        ...
        if (service == 'TRANSPORT' || service == 'ORDER') {
            ...
            for (var i in result) {
                if (result[i].vehicle) {
                    ...
                    btn = `<button class="btn btn-block btn-block btn-success view_vechile" data-value=` + service +
                        `  data-id =` + result[i].vehicle.id + `>View Vehicle</button>`;
                    html += `<tbody>
                                <tr>
                                <td>` + j + `</td>
                                <td>` + result[i].vehicle.vehicle_make + `</td>
                                 <td>` + btn + `</td>
                                 </tr>
                                 </tbody>`;
        ...
        if (totaldocuments[service] != undefined) {
            ...
            for (var i in result) {
                if (result[i].document) {
                    var j = parseInt(i) + 1;
                    btn = `<button class="btn btn-block btn-block btn-success view_button" data-value=` + service +
                        `  data-id =` + result[i].id + `>View Document</button>`;
                    html += `<tbody>
                            <tr>
                            <td>` + j + `</td>
    ...
    $('body').on('click', '.delete', function() {
        ...
        $.ajax({
            type: "DELETE",
            url: getBaseUrl() + "/admin/provider/delete_view_image/" + document_id + "?provider_id=" +
                id,
            headers: {
                Authorization: "Bearer " + getToken("admin")
            },
