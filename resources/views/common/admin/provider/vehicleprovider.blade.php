  ...
  <div class="col-md-12">
    <div class="card-header border-bottom">
      ...
      <button class="btn btn-block btn-block btn-success approve"
        style="width: 100px;position: absolute;right: 20px;top: 6px;">@lang('admin.provides.Approve')</button>
    </div>
        ...
        <div class="nav nav-tabs nav-fill " id="nav-tab " role="tablist">
          @foreach (Helper::getServiceList() as $k => $v)
            <a class="nav-item nav-link admin_list {{ $v }}" id="nav-daily-tab" data-toggle="tab"
              data-value={{ $v }} data-id="{{ $k }}" href="#{{ $v }}" role="tab"
              aria-controls="daily" aria-selected="true">{{ $v }}</a>
          @endforeach
        </div>
      </nav>
        ...
        success: function(data) {
          ...
          for (var i in result) {
            if (result[i].provideradminservice) {
              vechile_details[result[i].provideradminservice.providervehicle.id] = result[i]
        ...
        },
        {
          ...
          render: function(data, type, row) {
            return "<button data-id='" + data + "' id='" + data +
              "' class='btn btn-success btn-large view_button'>View</button> <button data-id='" + data +
