<div class="row mb-4 mt-20">
    <div class="col-md-12">
        <div class="card-header border-bottom">
            <h6 class="m-0">Provider Service Type Allocation</h6>
            <button class="btn btn-block btn-block btn-success approve"
                style="width: 100px;position: absolute;right: 20px;top: 6px;">{{ __('admin.provides.Approve') }}</button>
        </div>
            ...
            <nav>
                <div class="nav nav-tabs nav-fill " id="nav-tab " role="tablist">
                    @foreach (Helper::getServiceList() as $k => $v)
                        <a class="nav-item nav-link admin_list {{ $v }}" id="nav-daily-tab" data-toggle="tab"
                            data-value={{ $v }} data-id="{{ $k }}" href="#{{ $v }}"
                            role="tab" aria-controls="daily" aria-selected="true">{{ $v }}</a>
                    @endforeach
                </div>
            </nav>
            <div class="tab-content pricing-nav nav-wrapper" id="nav-tabContent">
                @foreach (Helper::getServiceList() as $k => $v)
                    <div class="tab-pane fade show active" id="{{ $v }}" role="tabpanel"
        ...
        <div class="form_pad">
            <div class="form-row m-0">
                ...
                <div class="form-group col-md-2">
                    <label for="zone_id">Action</label>
                    <button type="button" class="btn btn-accent update float-right">Update</button>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
                ...
                success: function(data) {
                    ...
                    for (var i in result) {
                        if (result[i].provideradminservice) {
                            vechile_details[result[i].provideradminservice.providervehicle
                                    .id] = result[i]
                                .provideradminservice.providervehicle;
                            btn =
                                `<button class="btn btn-block btn-block btn-success view_vechile" data-value='TRANSPORT'  data-id =` +
                                result[i].provideradminservice.providervehicle.id + `>View Vehicle</button>
                        <button class="btn btn-block btn-block btn-danger offbutton" data-value='TRANSPORT'  id =` +
                                result[i].provideradminservice.id + `>Delete</button>`;
                            html += `<tbody>
                                <tr>
                                 <td>` + service_name + `</td>
                    ...
                    for (var i in result) {
                        if (result[i].provideradminservice) {
                            vechile_details[result[i].provideradminservice.providervehicle
                                ...
                                .provideradminservice.providervehicle;
                            btn =
                                `<button class="btn btn-block btn-block btn-success view_vechile" data-value='ORDER'  data-id =` +
                                result[i].provideradminservice.providervehicle.id + `>View Vechile</button>
                        <button class="btn btn-block btn-block btn-danger offbutton" data-value='ORDER'  id =` +
                                result[i].provideradminservice.id + `>Delete</button>`;
                            html += `<tbody>
                                <tr>
                                 <td>` + service_name + `</td>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var i in result) {
                        if (result[i].provideradminservice) {
                            btn =
                                ` <button class="btn btn-block btn-block btn-danger offbutton"  data-value='SERVICE'  id =` +
                                result[i].provideradminservice.id + `>Delete</button>`;
                            html += `<tbody>
                                <tr>
        ...
        $(document).on('click', '.view_button', function(e) {
            var id = $(this).data('id');
            e.preventDefault();
            $.get("{{ url('admin/provider/') }}/" + $(this).data('id') + "/view_image", function(
                data) {
        ...
        table = table.DataTable({
            ...
            "columns": [{
                ...
                },
                {
                    "data": "status",
                    render: function(data, type, row) {
                        return "<span data-id='" + row.id + "' id='state" + row.id + "'>" + row
                            .status + "</span>";
                    }
                },
                {
                    "data": "id",
                    render: function(data, type, row) {
                        return "<button data-id='" + data + "' id='" + data +
                            "' class='btn btn-success btn-large view_button'>View</button> <button data-id='" +
                            data +
                            "' class='btn btn-danger btn-large doc-delete delete'>Delete</button>";
                    }
                }
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(data.responseData, function(key, item) {
                    ...
                    $("#zone_id").append('<option value="' + item.id + '" ' + selected +
                        '>' + item.name +
                        '</option>');
                });
        ...
        $('.update').click(function() {
            ...
            if (zone_id) {
                $.ajax({
                    type: "GET",
                    url: getBaseUrl() + "/admin/provider/zoneapprove/" + provider_service_id +
                        "?zone_id=" + zone_id,
                    headers: {
                        Authorization: "Bearer " + getToken("admin")
                    },
