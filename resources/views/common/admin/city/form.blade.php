<div class="row mb-4">
    <div class="col-md-12">
        <div class="card-header border-bottom">
            ...
            @endif
            <h6 class="m-0" style="margin:10!important;">{{ $action_text }} City</h6>
        </div>
        <div class="form_pad">
    ...
    $(document).ready(function() {
        ...
        $('.validateForm').validate({
            ...
            messages: {
                ...
                admin_service: {
                    required: "Admin Service is required."
                },
                status: {
                ...
                $.each(data.responseData, function(key, item) {
                    if (item.country.length != 0) {
                        $("#country_id").append('<option value="' + item.country.id + '">' +
                            item.country.country_name + '</option>');
                    }
                });
            }
        });
        ...
        $('#state_id').on('change', function() {
            ...
            $.ajax({
                type: "GET",
                url: getBaseUrl() + "/admin/cities/" + state_id + "?id=" + $("input[name=id]")
                    .val(),
                headers: {
                    Authorization: "Bearer " + getToken("admin")
                },
