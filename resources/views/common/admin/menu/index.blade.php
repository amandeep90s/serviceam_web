      ...
      <div class="col-md-12">
        <div class="card card-small">
          <div class="card-header border-bottom">
            <h6 class="m-0 pull-left">Menu</h6>

            <a href="javascript:;" class="btn btn-success pull-right add"><i class="fa fa-plus"></i> Add New Menu</a>
          ...
          <table id="data-table" class="table table-hover table_width display">
            <thead>
              <tr>
                <th data-value="sort_order">S.No</th>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('body').on('click', '.city', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/menucity/') }}/" + $(this).data('id') + "/" + $(this).data('service'), function(
          data) {
          $('.crud-modal .modal-container').html("");
          $('.crud-modal .modal-container').html(data);
        });
      ...
      table = table.DataTable({
        ...
        "columns": [{
...

          {
            "data": "id",
            render: function(data, type, row) {
              return " <button data-id='" + data + "' data-service='" + row.adminservice.display_name +
                "' class='btn btn-block btn-outline-primary city'>Menu City</button>";
            }
          },
          {
            "data": "id",
            render: function(data, type, row) {
              ...
              button += '<li><a href="javascript:;" data-id="' + data +
                '" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li><li><a href="javascript:;" data-id="' +
                data + '" class="dropdown-item delete"><i class="fa fa-trash"></i>&nbsp;Delete</a> </li>';
              button += '</ul> </li></div>';
              return button;
    ...
    function menuSelectCities(id) {
      ...
      $.ajax({
        ...
        success: function(data) {
          var countryCityList = '';
          countryCityList += `<a class="list-group-item citylist" href="#" onclick="getData(` + id +
            `,'ALL','ALL')"><span>All Countries</span></a>`;
          $.each(data.responseData, function(key, value) {
            ...
            countryCityList += `<a class="list-group-item  ` + countryActiveClass + ` citylist` + value.country
              .id + `" href="#" onclick="getData(` + id + `,` + value.country.id + `,'` + value.country
              .country_name + `')" id="` + value.country.id + `"><span>` + value.country.country_name +
              `</span></a>`;
