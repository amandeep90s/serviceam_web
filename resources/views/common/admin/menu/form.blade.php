<div class="row mb-4">
    <div class="col-md-12">
        <div class="card-header border-bottom">
            ...
            @endif
            <h6 class="m-0"style="margin:10!important;"> {{ $action_text }} Menu</h6>
        </div>
        <div class="form_pad">
            <form class="validateForm">
                ...
                <div class="form-row">
                    @if (count(Helper::getServiceList()) > 0)
                        <div class="form-group col-md-6">
                            <label for="notify_type"
                                class="col-xs-2 col-form-label">{{ __('admin.menu.flow_type') }}</label>
                    ...
                    @endif
                    <div class="form-group col-md-6">
                        <label for="menu_type_id"
                            class="col-xs-2 col-form-label">{{ __('admin.menu.menu_type') }}</label>
                ...
                <input type="hidden" value="{{ old('title') }}" name="title" required id="title"
                    placeholder="Title">
                @csrf()
                @if (!empty($id))
                ...
                <div class="form-group col-md-6 pull-right">
                    ...
                    <div class="btn btn-danger float-right cancel-image">Cancel</div>
                </div>
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <label for="bg_color" class="col-xs-2 col-form-label">{{ __('admin.menu.bg_color') }}</label>
                        <div class="col-xs-10">
                            <input style="width: 80px;" autocomplete="off" type="color" value="{{ old('bg_color') }}"
                                name="bg_color" required id="bg_color" placeholder="BG Color">
                        </div>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="sort_order" class="col-xs-2 col-form-label">Sort Order</label>
                        <div class="col-xs-10">
                            <input style="width: 150px;" class="form-control" autocomplete="off" type="number"
                                value="{{ old('sort_order') }}" name="sort_order" required id="sort_order"
                ...
                <div class="form-row">
                    <div class="form-group col-md-10">
                        <label for="is_featured"><input type="checkbox" name="is_featured" value=""
                                id="is_featured">&nbsp;{{ __('admin.menu.is_featured') }}</label>
                ...
                <button type="submit" class="btn btn-accent">{{ $action_text }} Menu</button>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data.responseData, function(key, item) {
                    $("#menu_type_id").append('<option value="' + item.id + '">' + item
        ...
        $('#service_id').change(function() {
            ...
            if ($service_id == 'TRANSPORT') {
                $.ajax({
                    ...
                    success: function(data) {
                        $("#menu_type_id").empty().append(
                            '<option value="">Select</option>');
                        $.each(data.responseData, function(key, item) {
                            $("#menu_type_id").append('<option value="' + item.id +
                                '">' + item.ride_name +
                                '</option>');
                        });
            ...
            $.ajax({
                ...
                success: function(response, textStatus, jqXHR) {
                    ...
                    if (data.is_featured == 1) {
                        ...
                        $('input[name=featured_image]').closest('.image-placeholder').find('img')
                            .attr('src', data
                                .featured_image);
                    }
                    $('input[name=picture]').closest('.image-placeholder').find('img').attr('src',
                        data.icon);
