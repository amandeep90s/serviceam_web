...
<div class="row mb-4 mt-20">
  <div class="col-md-12">
    <div class="card-header border-bottom">
      <h6 class="m-0"style="margin:10!important;"> City List</h6>
    </div>
    <div class="form_pad">
      <form class="validateForm">
        ...
        @if (Helper::getDemomode() != 1)
          <button type="submit" class="btn btn-accent float-right menucity">Add</button>
        @endif
      </form>
    </div>
  </div>
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('.menucity').on('click', function(e) {
      e.preventDefault();
      var cities = $('body').find(".validateForm input[name=city_id]:checked").map(function() {
        return $(this).val();
      }).get();
      ...
      "columns": [{
        ...
        },
        {
          ...
          render: function(data, type, row) {
            ...
            return `<a data-id='  ` + data + `'> <input type="checkbox" id="` + data + `"` + selected +
              ` value="` + row.city.id + `" name="city_id"> </a>`;
          }
        }
      ],
      responsive: true,
