<div class="row mb-4">
    <div class="col-md-12">
        <div class="card-header border-bottom">
            ...
            @endif
            <h6 class="m-0" style="margin:10!important;"> {{ $action_text }} Payroll</h6>
        </div>
        <div class="form_pad">
            <form class="validateForm" files="true">
                ...
                <div class="row">
                    <table id="data-table-provider" class="table table-hover table_width display">
                        <thead>
                            <tr>
                                <th><input type="checkbox" checked name="select_all" value="1"
                ...
                <button type="submit" class="btn btn-accent">{{ $action_text }} Payroll</button>
        ...
        poptable = poptable.DataTable({
            ...
            "columns": [{
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        return '<input type="checkbox" name="pid[]" checked value="' + $(
                                '<div/>').text(data).html() +
                            '"><input type="hidden" name="proid[' + row.id +
                            ']" checked value="' + row.provider_id +
                            '">';
                    }
                },
                {
                ...
                /*{ "data": "provider_id" },*/
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        return '<input class="form-control" id="wallet" name="wallet[' + row
                            .id +
                            ']" type="number" max="' + data + '"  min="1"  value = ' + data +
                            '  ><input class="form-control" id="template_id" name="template_id" type="hidden"  value = ' +
                            row.template_id +
                            '  ><input class="form-control" id="zones" name="zones[' + row.id +
                            ']" type="hidden"  value = ' + row.zone_id + '  >';
                    }
                },
                {
                    "data": function(data, type, row) {
                        ...
                        if ({{ Helper::getDemomode() }} != 1) {
                            var button = '<a href="javascript:;" data-id="' + data.id +
                                '" class="dropdown-item popdelete"><i class="fa fa-trash"></i> Delete</a> ';
                        }
                        return button;
