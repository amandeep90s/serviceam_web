...
@section('title') Add Geo Fencing @stop
            ...
            <div class="col-md-12">
                <div class="card card-small">
                    <div class="card-header border-bottom">
                        <h6 style="padding: 10px 0;" class="m-0 pull-left">
                            @if (isset($id))
                                Edit
                            @else
                                Add
                        ...
                        <div style="margin:16px 16px 0" class="form-group row">
                            ...
                            <div class="form-group col-md-4">
                                <label for="country_id"
                                    class="col-xs-2 col-form-label">{{ __('admin.city.country') }}</label>
                            ...
                            <div class="form-group col-md-4">
                                <label for="location_name"
                                    class="col-xs-2 col-form-label">{{ __('admin.geo_fencing.location') }}</label>
                                <input class="form-control" type="text" value="" name="location_name" required
                                    id="location_name" placeholder="Location">
                            </div>
...
@section('scripts')
    @parent
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ Helper::getSettings()->site->browser_key }}&libraries=places,drawing">
    </script>
    <script>
        var map;
        var polygon;
            ...
            google.maps.event.addListener(drawingManager, 'overlaycomplete', function(e) {
                ...
                if (e.type == google.maps.drawing.OverlayType.POLYGON) {
                    ...
                    $.each(locations, function(index, value) {
                        ...
                        layer_bounds.push(new google.maps.LatLng((value.lat()).toFixed(6), (value.lng())
                            .toFixed(6)));
                        json = {
                            'lat': markerLat,
                            'lng': markerLng
                ...
                success: function(data) {
                    ...
                    $.each(data.responseData, function(key, item) {
                        if (item.country.length != 0) {
                            $("#country_id").append('<option value="' + item.country.id + '">' +
                                item.country
                                .country_name + '</option>');
                        }
                    });
            ...
            $('#country_id').on('change', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.responseData, function(key, item) {
                            $("#city_id").append('<option value="' + item.city.id +
                                '">' + item.city.city_name +
                                '</option>');
                        });
                        setCity();
