    ...
    <script>
            ...
            success: (data, textStatus, jqXHR) => {
                ...
                for (var i in response) {
                    if (response[i].providerservices) {
                     ...
                     <a class="nav-link  tabs ` + response[i].admin_service + `" data-value="` + response[i]
                            .admin_service + `" data-toggle="tab" href="#` + response[i].admin_service +
                            `" role="tab" data-toggle="tab">` + response[i].admin_service + `</a>
                  </li>`;
                    }
                }
                    ...
                    for (var i in response) {
                        ...
                        if (response[i].provider_document) {
                            if (response[i].file_type == "image") {
                                image = `<img class ='add-document' src =` + response[i].provider_document.url[
                                    '0'].url + ` style='width: 100px;height:100px;'> `;
                            } else {
                                image = `<a  target="blank" href=` + response[i].provider_document.url['0']
                                    .url +
                                    `><img class="pdf" src="{{ asset('assets/layout/images/common/svg/pdf-file.svg') }}" alt="add_document"><div class="fileUpload up-btn profile-up-btn"><div data-name="front"><input type="file" id="profile_img_upload_btn"  class="upload"  accept="application/pdf"></div></div></a>`;
                            }
                        } else {
                            if (response[i].file_type == "image") {
                       ...
                       <input type='hidden' class='document_length' value=""></div>`;
                                ...
                                image =
                                    `<img class="pdf" src="{{ asset('assets/layout/images/common/svg/pdf.svg') }}" alt="add_document"><div class="fileUpload up-btn profile-up-btn"><input type="file" id="profile_img_upload_btn" class="upload"   accept="application/pdf"></div>`;
                            }
                        }
                        ...
                        if (response[i].provider_document) {
                            ...
                            if (response[i].is_backside == 1) {
                                ...
                                backendImage =
                                    ` <div class="col-sm-12 col-md-4 col-xl-4 pl-0  mt-2"> <div class="c-pointer"> <div class="add-document w-100">`;
                                if (response[i].file_type == "image") {
                                    backendImage += `<img class ='add-document' src =` + response[i]
                                        .provider_document.url['1'].url +
                                        ` style='width: 100px;height:100px;'>`;
                                } else {
                                    backendImage += `<a  target="blank" href=` + response[i].provider_document
                                        .url['1'].url +
                                        `><img class="pdf" src="{{ asset('assets/layout/images/common/svg/pdf-file.svg') }}" alt="add_document"><div class="fileUpload up-btn profile-up-btn"><div data-name="front"><input type="file" name="front" accept="application/pdf" /></div></div></a>`;
                                }
                                backendImage +=
                                    `</div><div><h5 class="p-0 text-center"><strong>Back</strong></h5></div></div></div>`;
                            } else {
                                backendImage =
                                    `<div class="col-sm-12 col-md-4 col-xl-4 pl-0  mt-2"> <div class="c-pointer"> <div class="add-document w-100"></div> <div><h5 class="p-0 text-center"><strong>No Back Image</strong></h5></div> </div></div>`;
                            }
                            ...
                            if (response[i].is_backside == 1) {
                                backendImage =
                                    ` <div class="col-sm-12 col-md-4 col-xl-4 pl-0  mt-2"> <div class="c-pointer"> <div class="add-document w-100"> <input type='hidden' class='is_backside' value="1"> `;
                                if (response[i].file_type == "image") {
                                    backendImage +=
                                        `<div data-name="back" class="image-placeholder w-100"> <img width="100" height="100" /> <textarea style="display:none"></textarea> <input type='hidden' name='file_name' value=""> <input type="file" name="picture" class="upload-btn picture_upload"   accept="image/x-png,image/gif,image/jpeg" > </div>`;
                                } else {
                                    backendImage +=
                                        `<img class="pdf" src="{{ asset('assets/layout/images/common/svg/pdf.svg') }}" alt="add_document"><div class="fileUpload up-btn profile-up-btn"><input type="file" name="front" accept="application/pdf"/></div>`;
                                }
                                backendImage +=
                                    `</div><div><h5 class="p-0 text-center"><strong>Back</strong></h5></div></div></div>`;
                            } else {
                        ...
                        if (response[i].provider_document) {
                            date =
                                `<div class="col-sm-12 col-md-4 col-xl-4 pl-0  mt-2 "> <h5 class=" no-padding"><strong>Expiry date</strong></h5> <input class="form-control" type="date" name="expires_at" id="expires_at" placeholder="Expiry Date" value = ` +
                                response[i].provider_document.expires_at +
                                `required onkeypress="return false" onkeypress="return false">`;
                            if (response[i].file_type == "image") {
                                button =
                                    `<a class="btn btn-danger  edit-profile mt-2 delete_image" data-type='` +
                                    response[i].type + `' data-id =` + response[i].provider_document.id +
                                    ` >Delete <i class="fa fa-trash" aria-hidden="true"></i></a></div>`;
                            } else {
                                button = `<a class="btn btn-danger  edit-profile mt-2 delete_pdf" data-type='` +
                                    response[i].type + `' data-id =` + response[i].provider_document.id +
                                    ` >Delete <i class="fa fa-trash" aria-hidden="true"></i></a></div>`;
                            }
                        } else {
                            date =
                                `<div class="col-sm-12 col-md-4 col-xl-4 pl-0  mt-2"> <h5 class=" no-padding"><strong>Expiry date</strong></h5> <input class="form-control" type="date" name="expires_at" id="expires_at" placeholder="" onkeypress="return false">`;
                            if (response[i].file_type == "image") {
                                button =
                                    `<a class="btn btn-primary edit-profile mt-2  upload_image" data-type='` +
                                    response[i].type +
                                    `' >Save <i class="fa fa-check" aria-hidden="true"></i></a></div>`;
                            } else {
                                button =
                                    `<a class="btn btn-primary edit-profile mt-2  upload_pdf" data-type='` +
                                    response[i].type +
                                    `' >Save <i class="fa fa-check" aria-hidden="true"></i></a></div>`;
                            }
                        }
        ...
        $('body').on('click', '.upload_image', function() {
            ...
            $(this).closest('.uploader').find('textarea').each(function(i) {
                if ($(this).val() != "") data.append(`file[` + i + `]`, blobToFile(($(this).val()).replace(
                    /data:image\/png;base64,/g, ''), $(this).parent().find(
                    'input[name=file_name]').val(), "image/png"), $(this).parent().find(
