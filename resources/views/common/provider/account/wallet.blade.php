            ...
            success: function(data) {
                ...
                $.each(result, function(key, item) {
                    ...
                    $.each(data.responseData, function(key, item) {
                        $("#card_id").append('<option value="' + item.card_id +
                            '"> **** **** **** ' + item.last_four + '</option>');
                    });
                });
            }
        });
        ...
        payment_table = payment_table.DataTable({
            ...
            "ajax": {
                ...
                data: function(data) {
                    ...
                    data.order_by = $('#payment_grid tr').eq(0).find('th').eq(data.order[0]['column']).data(
                        'value');
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        $.each(result, function(key, item) {
                            ...
                            transacts = transacts + `<tr><td>` + item.transaction_desc +
                                `</td><td>` + typename + `</td><td>` + item.amount + `</td></tr>`;
                        });
                        var result =
                            ` <div class="btn btn-xs btn-info collapsed" data-toggle="collapse" data-target="#modal` +
                            alias + `" data-id = ` + data +
                            ` data-toggle="tooltip" title="View" style="padding:5px !important;"><i class="fa fa-eye"></i></div> ` +
                            row.transaction_alias;
