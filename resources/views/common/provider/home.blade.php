	...
	<section class="p-0 dis-column  online-msg content-box mamdatory_section ">
		<div class="col-xl-6 col-sm-12 col-md-12 mamdatory_div  dis-column bg-body p-5">
			...
			<ul class="featureList">
			    <li class="tick service"></li>
				<li class="tick document"></li>
				<!-- <li class="tick bankdetail"></li>
				<li class="tick minwalletbalance"></li> -->
			</ul>
...
var providerdata={};
		$.ajax({
			...
			success:function(data){
				...
				}else if(providerdata.is_document !=0 && providerdata.is_service !=0 ) {
					$('.mamdatory_div').hide();
				...
				$('.document').append('<a class="txt-primary" href="{{ url("provider/document/ALL") }}"> Add Documents</a>');
				} else {
					$('.document').append('<span> Add Documents </span>');
				}

				/*if(providerdata.is_bankdetail==0){
					$('.bankdetail').removeClass('tick');
					$('.bankdetail').addClass('cross');
					$('.bankdetail').append(' <a class="txt-primary" href="{{ url("provider/profile/payment_method") }}"> Add Bank Details</a>');
				} else {
					$('.bankdetail').append('<span> Add Bank Details </span>');
				}*/

				if(providerdata.is_service==0){
					$('.service').removeClass('tick');
					$('.service').addClass('cross');
					$('.service').append(' <a class="txt-primary" href="{{ url("provider/myservice") }}"> Add Services</a>');
				} else {
					$('.service').append('<span> Add Services </span>');
				}
				...
				/*if(providerdata.is_walletbalance_min==0){
					...
					$('.minwalletbalance').append(' <a class="txt-primary" href="{{ url("provider/wallet") }}"> Add Money In Wallet</a>');
					$('.mamdatory_section').show();
					$('#booking-form').addClass('d-none');
				} else {
					$('.minwalletbalance').append('<span> Add Money In Wallet </span>');
				}*/
