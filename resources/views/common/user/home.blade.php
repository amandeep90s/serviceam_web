...
<body class='index'>
    ...
    <section class="services-list">
        ...
        <div class="result-show">
            <div class="col-xl-2 col-md-4 col-sm-12 location dis-center">
                <a class="btn btn-primary @if (Helper::getDemomode() != 1) dropdown-toggle @endif location-dropdown current_city"
                    data-toggle="dropdown" id="dropdownMenuButton">
                </a>
                @if (Helper::getDemomode() != 1)
                    <div class=" dropdown-menu city_list"></div>
                @endif
            </div>
            @if (Helper::getDemomode() == 1)
                <div class="col-sm-12" style="text-align:center"> <span style="color:red">** Demo Mode :
                        {{ __('admin.demomode_city') }} </span> </div>
            @endif
        ...
        <div class="container-fluid">
            <div class="dis-column mb-4">
                <hr>
                <h5 class="service-heading txt-primary">Services</h5>
            </div>
            <div class="row">
                <div class="col-sm-12">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <div id = "quantity_value" class="d-none">
                            <p>Quantity</p>
                            <input type ="text" class="form-control" value="1" name="quantity" minlength="1"
                                id="quantity" />
                ...
                <div class="modal-footer">
                    <a class="btn btn-primary btn-block service_id">Next <i class="fa fa-arrow-right"
    ...
    <div class="ongoing-service">
        ...
        <div class="service-text dis-ver-center p-0">
            <div class="active" onclick="openToggle()">
                <span id="heading" class="slider-heading">On Going Services</span>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        window.socket_url = 'http://localhost:8990';
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: (response, textStatus, jqXHR) => {
                    ...
                    $(".current_city").text(
                        result.filter((item) => item.id == userSettings.city_id).length > 0 ?
                        result.filter((item) => item.id == userSettings.city_id)[0]
                        .city_name :
                        ""
                    );
                    for (var i in result) {
                        $(".city_list").append(
                            `<a class="dropdown-item" style="cursor:pointer" data-id="` +
                            result[i].id +
                            `">` +
                            result[i].city_name +
                            `</a>`
            ...
            socket.emit(
                "joinCommonUserRoom",
                `room_${window.room}_${getUserDetails().id}_USER`
            );
                ...
                if (sub_category_id != "" && service != "") {
                    if (service_quantity == 1) {
                        window.location.replace(
                            "/user/service/" + service + "/service?qty=" + quantity
                        );
                    } else {
            ...
            $("#service").on("change", function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $("#quantity").empty().append('<option value="">Select</option>');
                        $.each(data.responseData, function(key, item) {
                            ...
                            $("#quantity").attr("maxlength", item.max_quantity);
                        });
                        hideInlineLoader();
                    }
                });
                              ...
                              <ul class="provider-list invoice">
                                 <li class="row">
                                    <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 p-0">
                                          <img src=` +
                                    item.image +
                                    ` height="100px" width="100px"  class="user-img">
                                       </div>
            ...
            $.ajax({
                ...
                success: (response, textStatus, jqXHR) => {
                    ...
                    if (result.length > 0) {
                        ...
                        <div class="service-item item-shadow" style="background-color:` +
                                    result[i].bg_color +
                                    `">
                        ...
                        <div class="service-content">
                              <h3 class="service-title">` +
                                 ...
                                 <a href="` +
                                    url +
                                    `/` +
                                    result[i].menu_type_id +
                                    `/transport" class="link-arrow">Book Now<i class="icon ion-ios-arrow-right"></i></a>
                        </div>
                        </div>
                     </div>`
                                );
                        ...
                        <div class="service-content">
                              <h3 class="service-title">` +
                                 ...
                                 <a href="` +
                                    ...
                                    `/` +
                                    result[i].menu_type_id +
                                    `" class="link-arrow">Book Now<i class="icon ion-ios-arrow-right"></i></a>
                        </div>
                        </div>
                     </div>`
                                );
                        ...
                        <div class="service-section serviceBlock" id =category_id data-id =` +
                                    result[i].menu_type_id +
                                    `>
                        <div class="service-content">
                              <h3 class="service-title">` +
                    ...
                    if (result.length > 0) {
                        ...
                        for (var i in result) {
                            if (result[i].service.admin_service == "TRANSPORT") {
                                html +=
                                    `<li>
                        <h6 class="txt-yellow"><strong>` +
                                    result[i].service.admin_service +
                                    `</strong></h6>
                        <div class="bg-lit-yellow text-left">
                              <h5 class="txt-white">From : ` +
                                    result[i].request.s_address +
                                    `</h5>
                              <h5 class="txt-white">To : ` +
                                    result[i].request.d_address +
                                    `</h5>
                              <br />
                              <a class="btn btn-primary" href="{{ url('/user/ride') }}/` +
                                    result[i].request.ride_type_id +
                                    `/transport?id=` +
                                    result[i].request.id +
                                    `">View</a>
                        </div>
                            ...
                            if (result[i].service.admin_service == "ORDER") {
                                html +=
                                    `<li>
                    <h6 class="txt-red"><strong>` +
                                    result[i].service.admin_service +
                                    `</strong></h6>
                    <div class="bg-lit-red text-left">
                        <h5 class="txt-white">shop Name : ` +
                                    result[i].request.pickup.store_name +
                                    `</h5>
                        <h5 class="txt-white">Booking ID : ` +
                                    result[i].request.store_order_invoice_id +
                                    `</h5>
                        <h5 class="txt-white">Status : ` +
                                    result[i].request.status +
                                    `</h5>
                        <br />
                        <a class="btn btn-red" href="{{ url('user/store/order') }}/` +
                                    result[i].request.id +
                                    `">View</a>
                    </div>
                </li>`;
                            ...
                            if (result[i].service.admin_service == "SERVICE") {
                                html +=
                                    `<li>
                    <h6 class="txt-blue"><strong>` +
                                    result[i].service.admin_service +
                                    `</strong></h6>
                    <div class="bg-lit-blue text-left">
                        <h5 class="txt-white">Service Location : ` +
                                    result[i].request.s_address +
                                    `</h5>
                        <h5 class="txt-white">Booking ID : ` +
                                    result[i].request.booking_id +
                                    `</h5>
                        <h5 class="txt-white">Status : ` +
                                    result[i].request.status +
                                    `</h5>
                        <br />
                        <a class="btn btn-blue" href="{{ url('/user/service') }}/` +
                                    result[i].request.service_id +
                                    `/service?id=` +
                                    result[i].request.id +
                                    `">View</a>
                    </div>
