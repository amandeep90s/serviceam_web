      ...
      $(document).ready(function() {
        ...
        $.ajax({
          ...
          success: (response, textStatus, jqXHR) => {
            ...
            $('.current_city').text((result.filter((item) => item.id == userSettings.city_id)
              .length > 0) ? result.filter((item) => item.id == userSettings.city_id)[
              0].city_name : '');
            for (var i in result) {
              $('.city_list').append(
                `<a class="dropdown-item" style="cursor:pointer" data-id="` + result[i]
                .id + `">` + result[i].city_name + `</a>`);
            }
          },
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              $.each(data.responseData, function(key, item) {
                $("#service").append('<option value="' + item.id + '">' +
      ...
      $('body').on('click', '.serviceBlock', function(e) {
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            $.each(data.responseData, function(key, item) {
              $("#sub_category").append('<option value="' + item.id + '" data-id="' +
                item.service_category_id + '">' + item
                ...
                if (result[i].service.admin_service == "TRANSPORT") {
                  html += `<li>
                        ...
                        <div class="bg-lit-yellow text-left">
                              ...
                              <br />
                              <a class="btn btn-primary" href="{{ url('/user/ride') }}/` + result[i].request
                    .ride_type_id + `/transport?id=` + result[i].request.id + `">View</a>
                        </div>
                     </li>`;
                }
                ...
                if (result[i].service.admin_service == "SERVICE") {
                  html += `<li>
                    ...
                    <div class="bg-lit-blue text-left">
                        ...
                        <h5 class="txt-white">Status : ` + result[i].request.status + `</h5>
                        <br />
                        <a class="btn btn-blue" href="{{ url('/user/service') }}/` + result[i].request.service_id +
                    `/service?id=` + result[i].request.id + `">View</a>
                    </div>
                </li>`;
                }
