...
$(document).ready(function() {
    ...
    table = table.DataTable( { 
        ...
        "columns": [
            ...
            { "data": function (data, type, row) {
                ...
                /*var button = "<button onclick='priceCountries("+data.id+")' data-toggle='modal' data-target='#service_type_service' class='btn btn-block btn-outline-primary price'>Price</button>";*/
                
                var button =  "<button data-id='"+data.id+"' class='btn btn-block btn-success edit'>Edit</button><button data-id='"+data.id+"' data-value='"+data.service_status+"' class='btn btn-block btn-warning status'>"+status+"</button>";

                if(data.created_type == 'PROVIDER')
                {
                    if(data.approved_status == 'APPROVED') {
                        var approved_status = "Approved";
                    } else {
                        var approved_status = "Not Approved";
                    }
                    button += "<button data-id='"+data.id+"' data-value='"+data.approved_status+"' class='btn btn-block btn-info approvedstatus'>"+approved_status+"</button>";
                }
    ...
    $('body').on('click', '.approvedstatus', function() {
        var id = $(this).data('id');
        var value = $(this).data('value');
         ...
         $(".status-modal").modal("show");
            $(".status-modal-btn")
                ...
                .on("click", function() {
                    ...
                    var url = getBaseUrl() + "/admin/service/listing/"+id+'/provider/service/updateStatus?status='+value;
...
function getData(serviceId,cityId,countryId){
