class MainComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        socket.emit('joinCommonProviderRoom', `room_${window.room}_nextProvider_${getProviderDetails().id}`);
        console.log('newww',`room_${window.room}_nextProvider_${getProviderDetails().id}`);
        socket.on('nextNewRequest',  (data) => {
    ...
    getStatus() {
        $.ajax({
            ...
            success: (response, textStatus, jqXHR) => {
                ...
                if(isFirefox && $('#root').children().length == 0) {
    ...
    render() {
        ...
        var nextrequest = this.state.nextRequest;

        console.log("heloooooo-----------"+nextrequest);
        if(this.state.account_status == "DOCUMENT" || this.state.account_status == "CARD" || this.state.account_status == "SERVICES" || this.state.account_status == "ONBOARDING") {
            ...
            if (status === "SEARCHING") {
                ...
                return <StartedComponent request={this.state.requests} provider={this.state.provider} FareType={this.state.FareType} nextRequest={this.state.nextRequest} />;
...
class StartedComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        var nextrequest = this.props.request.nextRequest;

        console.log("hiiii----"+nextrequest);
        ...
        if(status == "PICKEDUP" && nextrequest == 1){

            $("#nextrequestModal").modal("show");
            // $("#cancelModal").modal();
            // $('#nextrequest').modal('show');
        }
    ...
    handleOtpUpdate = (status, e) => {        
        ...
        let otp = $("input[name=otp]").val();
        let service_time_taken = $("input[name=service_time_taken]").val();
        console.log(service_time_taken);
        ...
        if((otpState == 1 && otp != "") || otpState == 0 && service_time_taken !="" && service_time_taken != 0) {
            $.ajax({
                url: getBaseUrl() + "/provider/update/serve/request",
                type: "POST",
                data: formdata,
            ...
            if(otp == ""){
                ...
                var alertTitle = 'Required';
                var alertMsg = 'Estimate time to complete the service';
                alertMessage(alertTitle, alertMsg, "danger");
            }
        }
    }
    ...
    <section className="z-1 pickup-section">
        ...
        <div className="modal" id="nextrequestModal">
                    ...
                    <form id="service_creation" encType="multipart/form-data">                 
                        ...
                        <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0 d-flex">
                            <div className="col-xs-12 col-sm-12 col-md-10 col-lg-11 p-0">
                                ...
                                <div className="">
                                    <input id="service_time_taken" className="form-control numbers" placeholder="Estimate Time to Complete(Within Minutes)" type="number" name="service_time_taken" />
                                </div>
                                :""}
